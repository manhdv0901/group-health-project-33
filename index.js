...
const SERVER_KEY =
  "AAAAqRousOQ:APA91bGX-6Wo0hGqvr9OrzCnX-8LEPNQXZsycdQR7qnrOH1Wzi5LDpo9UPyLNMayuL7F5QWXGI9wAxpRMwi7fOWRh3BrPHj9Nsc_5Fimt9Bb6wBO_GmbT97BDqXfZJNX4v2l_OXGAPsH";
...
app.post("/update/track-history/", (req, res) => {
  ...
  DEVICE.updateOne(
    { key_device: key },
    { $push: { track_history: { value: data, real_time: date } } },
    function (err) {
      if (!err) {
        // res.redirect('/list-patients');
        res.status(200).json({
...
app.post("/update/treatmentcourse/", (req, res) => {
  ...
  DEVICE.updateOne(
    { key_device: key },
    { $push: { treatment_course: { value: data, real_time: date } } },
    function (err) {
      if (!err) {
        // res.redirect('/list-patients');
        res.status(200).json({
